...
def run_update_critic_score(app, assistant_msg_id, conversation_history, search_history):
    """
        ...
        try:
            ...
            if assistant_msg:
                if isinstance(score, (int, float)) and score >= 0:
                    # Format as a simple critic score object for backward compatibility
                    critic_json = {
                        "total_score": score,
                        "summary": f"Automatically generated score: {score}"
                    }
                    assistant_msg.critic_score = json.dumps(critic_json)
                else:
                    # Score is -1 or otherwise invalid
                    assistant_msg.critic_score = json.dumps({
                        "total_score": 5.0,  # Default middle score
                        "summary": "Could not generate a valid critic score"
                    })
            ...
            try:
                # One more attempt with default values
                assistant_msg = db.session.get(AssistantMessage, assistant_msg_id)
                if assistant_msg:
                    assistant_msg.critic_score = json.dumps({
                        "total_score": 5.0,
                        "summary": f"Error generating score: {str(e)}"
                    })
                    assistant_msg.is_updating = False
                    db.session.commit()
            except:
...
class AssistantMessage(db.Model):
    ...
    critic_score = db.Column(db.Text, nullable=True)  # Store as JSON string
    is_updating = db.Column(db.Boolean, default=False, nullable=False)
    
    # New fields for enhanced processing
    thinking = db.Column(db.Text, nullable=True)  # Store the <think> content
    regenerated_content = db.Column(db.Text, nullable=True)  # Store the improved response
    regenerated_critic = db.Column(db.Text, nullable=True)  # Store the critique of the regenerated content
    ...
    def jsonify(self):
        ...
        if self.critic_score is not None:
            try:
                critic_data = json.loads(self.critic_score)
                if "total_score" in critic_data:
                    data["critic_score"] = critic_data["total_score"]
            except:
                pass
        return data
    ...
    def dump(self):
        ...
        if self.regenerated_content:
            data["regenerated_content"] = self.regenerated_content
        if self.regenerated_critic:
            data["regenerated_critic"] = self.regenerated_critic
